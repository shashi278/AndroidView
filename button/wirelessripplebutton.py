...
class WirelessRippleButton(Button):
	...
	def __init__(self, max_rad=500, circle_count=5, duration=.6, ripple_color=[52/255, 152/255, 219/255], **args):
	...
	def start_anim(self, *args):
		...
		self.vals=[self.max_rad/self.circle_count,(self.circle_count-1)*1/self.circle_count]

		anim= Animation(
			d=self.duration,
			**dict(zip(self.props, self.vals))
			)
	...
	def anim_intermediate(self, anim):
		...
		for i in range(1, self.circle_count):
			...
			self.vals= [(i+1)*self.max_rad/self.circle_count,(1-(i+1)/self.circle_count)]+self.vals
	...
	def anim_complete(self, obj, inst):
		...
		for i in range(self.circle_count-1, -1,-1):
			...
			if i==0:
				vals.extend([getattr(self, 'ripple_rad'+str(self.circle_count-1)),getattr(self, 'ripple_alpha'+str(self.circle_count-1))])
			else:
				vals.extend([getattr(self, 'ripple_rad'+str(i-1)),getattr(self, 'ripple_alpha'+str(i-1))])
